...
require 'Files/accessFile.php'; //files
?>
<!DOCTYPE html>
<html lang="en">
<head>
...
<?php
//check for input and token exists
if(Input::exists()){
    ...
    //get user input as a $uname
    $uname = Input::get('name');
        //if validation passed
    if($validation->passed()){
        $user1 = new User(); //create user object
        if($user1->find($uname)){ //attach user data by using find function
//            echo "User exist";
        ...
        <?php
            /*make session variables for phoneNumber, userID, flag */
            $_SESSION['phone'] = $user1->data()->phone;
            $_SESSION['id'] = $user1->data()->id;
            $_SESSION['flag'] = 1; //for send only one sms
        } else {
            //if user not exists
